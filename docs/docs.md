...
[Документация по группам параметров](param_groups.md) — раздельные lr и
weight decay для энкодера, декодера и обучаемых частей лосса. Зачем
предобученному энкодеру доля общего lr, почему BatchNorm не надо
регуляризовать и как это не сломать настройкой `eta_min`.
...
[Документация по PTQ](quantization.md) — fp32 -> fp16: ONNX, сборка движка
TensorRT под конкретную карту, три уровня валидации и замер производительности.
