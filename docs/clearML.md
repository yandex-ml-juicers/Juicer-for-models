...
## Что логирует этот репозиторий

Точка сборки — `clearml_reporter()` в `scripts/train.py`; тренер отдаёт ему данные
и сам про ClearML ничего не знает.

**Scalars** (каждую эпоху, ось X — номер эпохи): `loss` (train/eval),
`loss_components` (слагаемые лосса по отдельности), `lr`, `accuracy` или
`segmentation_metrics`/`pixel_accuracy`/`detection_metrics` — по типу задачи,
`grad_norm` и `weight_norm` (avg/max), `time_epoch`, `amp_skipped_steps`
(сколько шагов эпохи GradScaler отбросил из-за переполнения fp16).
Не-конечные значения не отправляются: ClearML подменяет `inf`/`NaN` нулём,
и в UI это выглядит как настоящий ноль метрики.

**Plots** (раздел Plots, `clearml.plots` в конфиге):

| График | Когда | Откуда данные |
|--------|-------|---------------|
| `per_class_iou` (train / eval) | каждую эпоху | накопленная confusion matrix |
| `grad_norm_distribution` | каждую эпоху | нормы градиента по шагам эпохи |
| `confusion_matrix` (eval) | раз в `every_n_epochs` + последняя эпоха | та же матрица, нормированная по строкам |
| `teacher_agreement_per_class` | каждую эпоху, если есть учитель | probe-шаги |
| `teacher_kl_distribution` | каждую эпоху, если есть учитель | probe-шаги |

**Debug Samples** — полоса `[кадр | разметка | предсказание | карта ошибок]`
по нескольким кадрам валидации, раздел *Debug Samples* задачи (серии
`predictions/sample_0…`). Кадры фиксированы на весь прогон и берутся с равным
шагом по датасету: Cityscapes отсортирован по городам, поэтому первые N подряд —
это N кадров одной улицы, а случайные каждый раз не с чем сравнивать. Первый
срез уходит **до начала обучения** (`iteration 0`) — необученная модель даёт шум,
и именно с ним потом сравниваются все эпохи; дальше раз в `debug_every_n_epochs`
и на последней эпохе. Разметка не закрашивает `ignore_index` (под ней видно
исходный кадр), void не считается ошибкой на четвёртой панели.

**Схожесть с учителем в сегментации** (`train_KL_divergence`, `train_agreement_rate`)
включается ключами `KL_divergence` / `agreement_rate` в `clearml.scalars`. Считается
не на каждом шаге и не по всем пикселям: `plots.probe_batches` замеров за эпоху
по `plots.probe_pixels` случайных пикселей на батч. Полный проход по кропу
512x1024 при batch=8 стоил бы softmax по ~80 млн значений — ради диагностики
это неоправданно, а подвыборка даёт ту же оценку.

**Собственное качество учителя** (график `segmentation_metrics`) — ДВА
НЕЗАВИСИМЫХ ключа в `clearml.scalars` (не один — чтобы можно было взять
дешёвую метрику без дорогой) — и отличается от предыдущего пункта: там —
насколько ученик ПОХОЖ на учителя, здесь — насколько сам учитель ПРАВ
относительно разметки. Полезно, когда учитель в KD получает не то, на чём
его меряли (например, кроп вместо родного разрешения) — тогда он сам отдаёт
ученику шум, и никакая архитектура/лосс ученика это не вытянут.

- `teacher_miou` -> `train_teacher_mIoU` — IoU учителя за эпоху ПО ТЕМ ЖЕ
  КРОПАМ/МАСШТАБУ/АУГМЕНТАЦИЯМ, что ПРЯМО СЕЙЧАС видит ученик при
  дистилляции (`teacher_logits` в `SegmentationTrainer._train_epoch`, ровно
  то, что уходит в KD-лосс). Почти бесплатно: эти логиты и так считаются
  каждый шаг, добавляется только `argmax`+`bincount`, лишнего forward'а нет.
  Это и есть ответ на вопрос "насколько сильные таргеты учитель выдаёт
  ученику прямо сейчас, пока учит его".
- `teacher_native_miou` -> `teacher_native_mIoU` — IoU учителя на
  `eval_loader` (родное разрешение, без кропа и без аугментаций — тот же
  путь, что `scripts/eval.py eval_slot=teacher`). Считается ОДИН раз в
  начале `fit()` (учитель заморожен, число не меняется по эпохам, и
  аугментаций на eval нет — оно и так известно заранее) и дальше просто
  повторяется в каждой строке лога как плоская линия-ориентир. НЕ включена
  автоматически вместе с `teacher_miou` — включайте отдельно, только если
  хотите готовую линию сравнения на том же графике, а не считать сами.
  Стоимость при включении — один проход по валидации (~1-2 минуты на
  Cityscapes/SegFormer-B5), разово на весь прогон. Батч этого прохода —
  ОТДЕЛЬНЫЙ ключ, `clearml.plots.teacher_native_eval_batch_size` (дефолт 4),
  а не `data.loader.eval_batch_size`: тот подобран под студента, и для
  тяжёлого учителя на 1024x2048 в нём легко случается OOM (проверено на
  SegFormer-B5 при eval_batch_size=14 — падает; 4 работает с запасом).

Разница между двумя линиями на одном графике — прямой ответ на вопрос "не
дистиллируем ли мы шум из-за несовпадения разрешения/контекста учителя".
